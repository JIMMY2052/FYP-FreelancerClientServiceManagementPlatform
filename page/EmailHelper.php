...
/**
 * PHPMailer Email Helper
 * Initializes and returns a configured PHPMailer instance
 */
...
function get_mail()
{
    ...
    $mail->Port = 587;
    $mail->SMTPAuth = true;
    $mail->Username = 'jimmychankahlok66@gmail.com';
    $mail->Password = 'kygf bpez eztp fdlp'; // App password
    $mail->CharSet = 'utf-8';

    // Sender
    $mail->setFrom('jimmychankahlok66@gmail.com', 'WorkSnyc');
...
function sendOTPEmail($email, $otp, $userName = null)
{
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = "WorkSnyc - Password Recovery OTP";
        ...
        <head>
            ...
            <style>
                ...
                .email-card { background-color: white; border-radius: 12px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
                ...
                h1 { color: #1f2937; font-size: 24px; margin: 20px 0 10px 0; }
                .otp-section { background-color: #f0fde8; border: 2px solid #22c55e; border-radius: 8px; padding: 25px; text-align: center; margin: 25px 0; }
                .otp-code { font-size: 36px; font-weight: bold; color: #22c55e; letter-spacing: 5px; font-family: 'Courier New', monospace; }
                .otp-label { color: #6b7280; font-size: 14px; margin-top: 10px; }
                .expiry-warning { background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 12px 15px; border-radius: 4px; margin: 20px 0; color: #92400e; }
                .instructions { background-color: #f3f4f6; padding: 15px; border-radius: 8px; margin: 20px 0; color: #374151; }
        ...
        $mail->send();
        error_log("[OTP Email] Successfully sent OTP to: $email");
        return true;
    } catch (Exception $e) {
        error_log("[OTP Email] Failed to send OTP to: $email. Error: " . $e->getMessage());
        return false;
    }
}
